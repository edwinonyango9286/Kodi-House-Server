...
// new access token should only be generated if the there is a refresh token
const verifyUserToken = expressAsyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (!user) {
      return res.status(401).json({status: "FAILED", message: "Invalid access token. Please log in to continue."});
    }
    req.user = user;
    next();
  } catch (error) {
