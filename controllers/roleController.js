...
const createARole = expressAsyncHandler(async (req, res, next) => {
  try {
    ...
    if (createdRole) {
      return res.status(201).json({status: "SUCCESS",message: "Role created successfully.",data: createdRole,});
    }
  } catch (error) {
    logger.error(error);
    next(error);
...
const grantPermissionToARole = expressAsyncHandler(async (req, res, next) => {
  try {
    ...
    if (!permissionId) {
      return res.status(400).json({ status: "FAILED", message: "Please provide permission Id." });
    }
    validateMongoDbId(permissionId);

    const grantedRole = await Role.findOneAndUpdate({_id: roleId,},{$addToSet: { permissions: permissionId },},{ new: true, runValidators: true });
    if (!grantedRole) { return res.status(400).json({ status: "FAILED", message: "Role not found." });}
